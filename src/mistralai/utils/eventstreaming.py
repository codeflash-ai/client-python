...
def _parse_event(
    ...
    block = raw.decode()
    # Split manually to avoid re.split regex overhead/high allocation
    # much faster for small line separator set, and avoids overhead of regex engine
    lines = block.splitlines()
    publish = False
    event = ServerEvent()
    # Use list for efficient string accumulation
    data_chunks = []
    for line in lines:
        if not line:
            continue

        # Find first colon efficiently; string ops are fast for this
        delim = line.find(":")
        if delim <= 0:
            continue

        # Avoid slicing repeatedly
        field = line[:delim]
        # .find guarantees delim >= 0, and delim+1 slices safely
        value = line[delim + 1 :]
    ...
    # Combine data for further checks
    if data_chunks:
        data = "\n".join(data_chunks) + "\n"
    else:
        data = ""
    ...
    if data:
        # Remove final newline inserted above
        data = data[:-1]
        event.data = data

        # These checks are very short, so no change to logic
        data_is_primitive = (
    ...
    out = None
    if publish:
        # json.dumps(event.__dict__) is unavoidable for decoder signature; fastest possible way
